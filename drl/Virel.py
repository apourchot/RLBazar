...
# 
            ...
            if it % self.policy_freq == 0:
                ...
                actor_loss -= torch.log(sigmas ** 2).mean(dim=1, keepdim=True) * 5 # critic_loss.detach()
...
class VirelKL(object):
    """
    VIREL-inspired Actor-Critic Algorithm: https://arxiv.org/abs/1811.01132 with different prior
    """
    ...
    def train(self, memory, n_iter):
        ...
        for it in range(n_iter):
            ...
            if it % self.policy_freq == 0:
                ...
                # actions, mus, sigmas
                actions, mus, sigmas = self.actor(states)
                actions_t, mus_t, sigmas_t = self.actor_t(states)

                print(torch.log(sigmas ** 2))
                print(torch.log(sigmas_t ** 2))

                # Compute actor loss with entropy
                actor_loss = -self.critic(states, actions)[0]
                print(actor_loss)
                actor_loss += (torch.log(sigmas_t ** 2) - torch.log(sigmas ** 2)).mean(dim=1)
                actor_loss += ((sigmas ** 4 + (mus - mus_t) ** 2) / (2 * sigmas_t ** 4)).mean(dim=1)
                actor_loss = actor_loss.mean()
